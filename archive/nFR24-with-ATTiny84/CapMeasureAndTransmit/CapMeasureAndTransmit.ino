...
void setup() {
  ...
  /*  Init capacitance measurement pins. 
      For now we're just going to create dummy data and
      have an actualy cap attached. */
  ...
  memcpy(txPayload.units, "--- ", 4);
  txPayload.capacitance = 0.0;
...
void loop() {
  ...
  if (report) { // IF report is true than nRF24 chip believes it sent the payload out.
      ...
      memcpy(txPayload.statusText, "Success ", 8);            // clear error status text
      ...
      if (radio.available(&pipe)) {                             // ACK if-then block. Test for ack payload, which will also return the pipe number that received it.
        ...
        memcpy(txPayload.statusText, "ERROR 02 ", 10);        // Set text to send back to RPi to show error.
    ...
    memcpy(txPayload.statusText, "ERROR 03 ", 10);        // Set text to send back to RPi to show error.
