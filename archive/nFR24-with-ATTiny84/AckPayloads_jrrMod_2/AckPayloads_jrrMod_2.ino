...
/*  6/17/2022: Trying to get Milestone #4 to work again.
 *   
...
void loop() {
  ...
  if (report) { // IF report is true than payload was transmitted out by the nRF chip and a auto-ack received back.
    ...
    digitalWrite(errorLedPin, HIGH);                // red LED on (so should then be on steady, but keep trying to transmit without delays)
    memcpy(payload.message, "xxxxx ", 6);           // if transmission received by RPi we'll see the xxxxx
    delay(500);                                     // add a little delay just 'cause
...
void errorLED (int errorNo) {
  digitalWrite(greenLedPin, LOW);           // turn LEDs off to prep for count-out
  digitalWrite(errorLedPin, LOW);
  delay(1500);
  ...
  delay(1500);                              // provide a clear end to the count
  digitalWrite(errorLedPin, HIGH);          // now exit with Red LED on steady
} // errorLED
