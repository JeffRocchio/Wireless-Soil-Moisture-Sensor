 ...
 * 07/19/2022: Cleaning up the console display approach with goal of honing in on success vs
               error rates. Ok, I have made a better display that now shows me the rate of
               errors and success. Interestingly, for reasons I cannot explain, when I
               now run this program I am getting a better successes to error ratio. Something
               in the air? Some subtly of timing that the new code has introduced that makes
               the syncing between the two radios work better? I have no idea.

 */
#define VERSION "07-19-2022 rel 02"
...
void slave() {
    ...
    while (time(nullptr) - startTimer < 12) {                           // use 12 second timeout
        ...
        if (radio.available(&pipe)) {                                   // is there a received payload? get the pipe number that recieved it
            ...
            //displayRxStruct(&rxPayload);                                // <-- IF below matches this display, remove this one
            displayRxResults(&rxPayload, true);                         // display received transmission info
 ...
 */
void displayRxResults(RxPayloadStruct* pStruct, bool bCurReset) {
