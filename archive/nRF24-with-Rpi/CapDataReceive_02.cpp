 ...
 *
 * 06-17-2022: First iteration.
 * 06-18-2022: Trying to debug incorrect display of the RxPayload.chargeTime.
 *
...
#define VERSION "06-18-2022 rel 05"
...
//trying to deal with endian situation
...
void master() {
    ...
    cout << endl;
/*
    memcpy(ackPayload.message, "Hello ", 6);                        // set the ackPayload message
    ...
    while (failures < 6) {
        ...
        if (report) {
            ...
            cout << " us. Sent: ";
            cout << ackPayload.message;                             // print outgoing message
            cout << (unsigned int)ackPayload.counter;               // print outgoing counter counter

            uint8_t pipe;
            if (radio.available(&pipe)) {
                // got an ACK payload
                PayloadStruct rxPayload;
                radio.read(&rxPayload, sizeof(rxPayload));          // get incoming ACK payload
...
void slave() {
    ...
    while (time(nullptr) - startTimer < 6) {                 // use 6 second timeout
        ...
        if (radio.available(&pipe)) {                        // is there a received payload? get the pipe number that recieved it
            ...
            cout << " Float-1: size - " << sizeof(rxPayload.testFloat1) << " value - " << (float)rxPayload.testFloat1;
